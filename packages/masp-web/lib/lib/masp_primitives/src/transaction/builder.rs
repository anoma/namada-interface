...
impl<P: consensus::Parameters, R: RngCore + CryptoRng> Builder<P, R> {
    ...
    pub fn add_sapling_spend(
        ...
        self.mtx.value_balance +=
            Amount::from(note.asset_type, note.value).map_err(|_| Error::InvalidAmount)?;
    ...
    pub fn add_sapling_output(
        ...
        self.mtx.value_balance -=
            Amount::from(asset_type, value).map_err(|_| Error::InvalidAmount)?;
    ...
    pub fn build(
        ...
        // Valid change
        let change = self.mtx.value_balance.clone() - self.fee.clone()
...
mod tests {
    ...
    fn fails_on_negative_change() {
        ...
        // 0.0005 z-ZEC out, 0.0001 t-ZEC fee
        {
            let mut builder = Builder::<TestNetwork, OsRng>::new(0);
            builder
                .add_sapling_output(ovk.clone(), to.clone(), zec(), 50000, None)
                .unwrap();
            assert_eq!(
                builder.build(consensus::BranchId::Sapling, &MockTxProver),
                Err(Error::ChangeIsNegative(
                    Amount::from(zec(), -60000).unwrap()
                ))
            );
        }
        ...
        // 0.0005 t-ZEC out, 0.0001 t-ZEC fee
        {
            let mut builder = Builder::<TestNetwork, OsRng>::new(0);
            builder
                .add_transparent_output(&TransparentAddress::PublicKey([0; 20]), zec(), 50000)
                .unwrap();
            assert_eq!(
                builder.build(consensus::BranchId::Sapling, &MockTxProver),
                Err(Error::ChangeIsNegative(
        ...
        // 0.0003 z-ZEC out, 0.0002 t-ZEC out, 0.0001 t-ZEC fee, 0.00059999 z-ZEC in
        {
            ...
            builder
                ...
                .unwrap();
            builder
                .add_sapling_output(ovk.clone(), to.clone(), zec(), 30000, None)
                .unwrap();
            builder
                .add_transparent_output(&TransparentAddress::PublicKey([0; 20]), zec(), 20000)
                .unwrap();
            assert_eq!(
                builder.build(consensus::BranchId::Sapling, &MockTxProver),
