...
impl SaplingProvingContext {
    ...
    pub fn spend_proof(
        ...
        let value_commitment2 = Some(value_commitment.clone());
        let proof_generation_key2 = Some(proof_generation_key);
        let payment_address2 = Some(payment_address);
        let commitment_randomness2 = Some(note.rcm());
        let ar2 = Some(ar);
        let auth_path2 = merkle_path
            .auth_path
            .iter()
            .map(|(node, b)| Some(((*node).into(), *b)))
            .collect();
        let anchor2 = Some(anchor);
        // We now have the full witness for our circuit
        let instance = Spend {
            value_commitment: value_commitment2,
            proof_generation_key: proof_generation_key2,
            payment_address: payment_address2,
            commitment_randomness: commitment_randomness2,
            ar: ar2,
