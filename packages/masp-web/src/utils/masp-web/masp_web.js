...
export function create_shielded_transfer(shielded_transactions, spending_key_as_string, payment_address_as_string, token_address, amount, spend_param_bytes, output_param_bytes) {
    try {
        ...
        var ptr0 = isLikeNone(spending_key_as_string) ? 0 : passStringToWasm0(spending_key_as_string, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
...
export class NodeWithNextId {
    ...
    free() {
        ...
        wasm.__wbg_nodewithnextid_free(ptr);
    }
    /**
    * @param {Uint8Array} transfer_as_byte_array
    * @returns {any}
    */
    static decode_transaction_with_next_tx_id(transfer_as_byte_array) {
        const ptr0 = passArray8ToWasm0(transfer_as_byte_array, wasm.__wbindgen_malloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.nodewithnextid_decode_transaction_with_next_tx_id(ptr0, len0);
        return takeObject(ret);
    }
}
...
export class ShieldedAccount {
    ...
    free() {
        const ptr = this.__destroy_into_raw();
        wasm.__wbg_shieldedaccount_free(ptr);
    }
    /**
    * @param {string} alias
    * @param {string} seed
...
function getImports() {
    ...
    imports.wbg.__wbg_log_653d66fe690ef32a = function(arg0, arg1) {
...
async function init(input) {
    if (typeof input === 'undefined') {
        input = new URL('masp_web_bg.wasm', import.meta.url);
    }
    const imports = getImports();
