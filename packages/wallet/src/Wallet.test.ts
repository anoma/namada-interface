...
describe("Wallet class", () => {
  ...
  test("It should derive the correct public keys from mnemonic", async () => {
    ...
    const child2 = walletBtc.deriveChildAccount(0, 1);
    ...
    const child3 = walletEth.deriveChildAccount(0, 0);
    const child4 = walletEth.deriveChildAccount(0, 1);
    const child5 = walletEth.deriveChildAccount(0, 2);
  ...
  test("Derives the correct addresses and private key WIFs from mnemonic", async () => {
    ...
    const child1 = wallet.deriveChildAccount(0, 0);
    const child2 = wallet.deriveChildAccount(0, 1);
  ...
  test("Can override the default encoding for private/public keys", async () => {
    const wallet = await new Wallet(MNEMONIC_24, "BTC").init();
    const child = wallet.deriveChildAccount(0, 0, "base58");
  ...
  test("ed25519 keypair can be serialized from derived account", async () => {
    ...
    const wallet = await new Wallet(MNEMONIC_24, "BTC").init();
    const child = wallet.deriveChildAccount(0, 0);
