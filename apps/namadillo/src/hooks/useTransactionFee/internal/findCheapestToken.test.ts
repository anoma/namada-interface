...
describe("findCheapestToken", () => {
  ...
  it("returns the token with the cheapest gas price in USD and sufficient balance", () => {
    const gasPriceTable = [
      { token: tokenA, gasPrice: new BigNumber(2) },
      { token: tokenB, gasPrice: new BigNumber(1) },
      { token: tokenC, gasPrice: new BigNumber(3) },
    ];

    const balance = [
      { tokenAddress: tokenA, amount: new BigNumber(5000) },
      { tokenAddress: tokenB, amount: new BigNumber(5000) },
      { tokenAddress: tokenC, amount: new BigNumber(5000) },
    ];
  ...
  it("skips cheaper tokens without sufficient balance", () => {
    const gasPriceTable = [
      { token: tokenA, gasPrice: new BigNumber(1) },
      { token: tokenB, gasPrice: new BigNumber(2) },
    ];

    const balance = [
      { tokenAddress: tokenA, amount: new BigNumber(500) },
      { tokenAddress: tokenB, amount: new BigNumber(5000) },
    ];
  ...
  it("returns undefined when no tokens have sufficient balance", () => {
    ...
    const balance = [
      { tokenAddress: tokenA, amount: new BigNumber(500) },
      { tokenAddress: tokenB, amount: new BigNumber(100) },
    ];
  ...
  it("returns undefined when gasDollarMap is missing entries", () => {
    const gasPriceTable = [{ token: tokenA, gasPrice: new BigNumber(1) }];

    const balance = [{ tokenAddress: tokenA, amount: new BigNumber(5000) }];
  ...
  it("returns the correct token when dollar values are equal but gas prices differ", () => {
    const gasPriceTable = [
      { token: tokenA, gasPrice: new BigNumber(2) },
      { token: tokenB, gasPrice: new BigNumber(3) },
    ];
