...
export const signTxArray = async <T>(
  ...
  try {
    ...
    if (typedEncodedTxs[0].type === revealPublicKeyType) {
      ...
      const tx = revealPk.tx as BuiltTx;
    ...
    // Sign remaining Tx as a batch
    const signedBatchTxBytes = await signingClient.signBatch(
